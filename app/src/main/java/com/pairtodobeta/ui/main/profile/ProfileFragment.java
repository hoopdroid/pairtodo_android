package com.pairtodobeta.ui.main.profile;
...
import com.pairtodobeta.ui.main.chat.models.DefaultDialog;
...
public class ProfileFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    CircularImageView userProfilePhoto;
    ...
    @Bind(R.id.noCoupleView)
    ViewGroup noCoupleView;
    @Bind(R.id.searchCoupleBtn)
    Button searchCoupleBtn;
    @Bind(R.id.userContent)
    ViewGroup userContent;
    @Bind(R.id.settings)
    ImageView settings;
    @Bind(R.id.chatLoading)
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (info != null) {
            ...
            Picasso.with(getActivity()).load(info.getPhoto())
    ...
    private void getMessageArchive() {
        if (getActivity() != null && isAdded()) {
            ...
            if (messages.size() > 0)
                ...
                initChatElements(Message.create("", "", "", "",
                        "", getString(R.string.start_conversation_with_couple), ""));
    ...
    private void initChatElements(Message lastMessage) {
        ...
        DefaultUser couple = new DefaultUser(info.getPairId(), info.getPairName(), info.getPairPhoto(), true);
        if (lastMessage.fromId().equals(info.getId()))
            lastMessage.setUserMessage(user);
        else lastMessage.setUserMessage(couple);

        Message secretStickerMessage;
        DefaultDialog defaultDialog;
        if (MessagesListAdapter.checkIfStickerEncode(lastMessage.getText())) {
            secretStickerMessage = Message.create(lastMessage.isRead(),
                    lastMessage.getId(), lastMessage.toId(), lastMessage.jobId(),
                    lastMessage.id(), getString(R.string.sticker), lastMessage.createDate()
            );
            if (lastMessage.fromId().equals(info.getId()))
                secretStickerMessage.setUserMessage(user);
            else secretStickerMessage.setUserMessage(couple);

            defaultDialog = new DefaultDialog("1", getActivity().getString(R.string.dialog_with) + " " + couple.getName(),
                        ...
                        if (response.getUserInfo() != null && getActivity() != null) {
                            if (info != null)
                                if (!response.getUserInfo().getPhoto().equals(info.getPhoto()))
                                    Picasso.with(getActivity()).load(response.getUserInfo().getPhoto())
                                            .into(userProfilePhoto);
                            if (info != null)
