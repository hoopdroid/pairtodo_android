...
public class BaseActivity extends AppCompatActivity {
    ...
    public void checkCoupleStatus() {
        ...
        handler.post(new Runnable() {
            @Override
            public void run() {
                Job job = JobMessageService.getInstance().checkCoupleStatus(new SmartScheduler.JobScheduledCallback() {
                    @Override
                    public void onJobScheduled(final Context context, Job job) {
                        if (job != null) {
