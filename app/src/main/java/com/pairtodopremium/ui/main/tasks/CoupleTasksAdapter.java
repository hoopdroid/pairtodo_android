...
public class CoupleTasksAdapter extends RecyclerView.Adapter<CoupleTasksAdapter.ViewHolder> {
  ...
  public CoupleTasksAdapter(Context context, List<Task> newsList, String executorId) {
    ...
    for (int i = 0; i < newsList.size(); i++) {
      if (!Objects.equals(tasksList.get(i).getExecutorId(), executorId)) tasksList.remove(i);
    }
  }
  ...
  @Override public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
    View view =
        LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_task, viewGroup, false);
  ...
  @Override public void onBindViewHolder(final ViewHolder viewHolder, int i) {
    ...
    if (!Objects.equals(tasksList.get(i).getTermDate(), "0")) {
      ...
      viewHolder.dueDateText.setReferenceTime(
          Long.parseLong(tasksList.get(i).getTermDate()) * 1000);
    ...
    if (tasksList.get(i).getTermDate().equals("0")) viewHolder.dueDateText.setVisibility(View.GONE);

    if (tasksList.get(i).getIsFinish().equals("1")) {
      viewHolder.postText.setPaintFlags(
          viewHolder.postText.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
      viewHolder.taskDescription.setPaintFlags(
          viewHolder.taskDescription.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
      viewHolder.dueDateText.setPaintFlags(
          viewHolder.dueDateText.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
    }
  ...
  private void addAnimationToPostItem(ViewHolder viewHolder, int i) {
    Animation animation = AnimationUtils.loadAnimation(context,
        (i > lastPosition) ? R.anim.up_from_bottom : R.anim.down_to_top);
  ...
  public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    @Bind(R.id.post_container) CardView cardView;
    @Bind(R.id.tv_name) TextView postText;
    @Bind(R.id.tv_due_date) RelativeTimeTextView dueDateText;
    @Bind(R.id.favorite_item_icon) ImageView isFavoriteIcon;
    @Bind(R.id.list_item_icon) ImageView isListIncludedIcon;
    @Bind(R.id.tv_task_descr) TextView taskDescription;
    ...
    @Override public void onClick(View v) {
      ...
      if (v == cardView) {
        ActivityRouter.startTaskActivity((Activity) context, tasksList.get(getPosition()),
            Constants.CHANGE_TASK);
      }
    }
  }
}