...
public class MyTasksAdapter extends RecyclerView.Adapter<MyTasksAdapter.ViewHolder> {
  ...
  public MyTasksAdapter(Context context, List<Task> newsList, String executor) {
    this.context = context;
    this.tasksList = newsList;
    for (int i = 0; i < newsList.size(); i++) {
      if (!newsList.get(i).getExecutorId().equals(executor)) newsList.remove(i);
    }
  }
  ...
  public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    @Override public void onClick(View v) {
      ...
      if (v == cardView) {
        Task task = tasksList.get(getPosition());
        ActivityRouter.startTaskActivity((Activity) context, task, Constants.CHANGE_TASK);
      }
    }
  }
}