...
import com.pairtodopremium.ui.main.chat.ChatSamplesListAdapter;
  ...
  private void loadMessagesArchive() {
    DataService.init().getJobMessages(new DataService.onMessageGetTask() {
      @Override public void onMessageResult(List<Message> messages) {
        ...
        for (int i = 0; i < messages.size(); i++) {
          if (Objects.equals(messages.get(i).jobId(), mTask.getId())) tempList.add(messages.get(i));
        }
        tempList.add(0,
            Message.create("", "", "", "", "", mTask.getTitle(), mTask.getCreateDate()));
