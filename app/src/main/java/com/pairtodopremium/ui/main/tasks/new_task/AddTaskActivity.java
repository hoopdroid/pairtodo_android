...
public class AddTaskActivity extends BaseActivity {
  ...
  @Bind(R.id.noteTitle) EditText noteTitleEdit;
  @Bind(R.id.note_description) EditText noteDescriptionEdit;
  @Bind(R.id.dut_date_text) TextView dueTextDate;
  @Bind(R.id.discrete_slider) DiscreteSlider discreteSlider;
  @Bind(R.id.tick_mark_labels_rl) RelativeLayout tickMarkLabelsRelativeLayout;
  @Bind(R.id.yourselfTask) TextView yourSelfTask;
  @Bind(R.id.coupleTask) TextView coupleTask;

  @Bind(R.id.show_subtask_view) ImageView showSubTaskView;
  @Bind(R.id.subtask_view) ViewGroup subtaskView;

  @Bind(R.id.add_list_note_item) ImageView listItemAddBtn;
  @Bind(R.id.list_note_item) EditText listNoteEdit;
  @Bind(R.id.currentList) TextView allSubTasksView;

  @Bind(R.id.photoTaskImage) ImageView photoTaskImage;
  @Bind(R.id.progressImage) ProgressBar progressImage;
  String pickedPhoto = "";
  ...
  private void initViewElements() {
    ...
    fab.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View view) {
        ...
        Task task = new Task(noteTitleEdit.getText().toString().trim(), dueDate, description, list,
            pickedPhoto, "0", String.valueOf(coupleTask.isSelected() ? 1 : 0));
        if (progressImage.getVisibility() != View.VISIBLE) {
          DataService.init().addTask(new DataService.onAddTask() {
            @Override public void onAddTaskResult(BasicResponse response) {
              setResult(RESULT_OK);
              finish();
            }

            @Override public void onAddTaskError() {
              // TODO Show error
            }
          }, task, PrefRepository.getToken(AddTaskActivity.this));
        } else {
    ...
    discreteSlider.setOnDiscreteSliderChangeListener(
        new DiscreteSlider.OnDiscreteSliderChangeListener() {
    ...
    tickMarkLabelsRelativeLayout.getViewTreeObserver()
  ...
  private void openDatePicker() {
    ...
    DatePickerDialog datePicker =
        new DatePickerDialog(this, android.R.style.Theme_Material_Dialog, datePickerListener,
  ...
  private void addTickMarkTextLabels() {
    ...
    int interval =
        (width - (discreteSliderBackdropLeftMargin + discreteSliderBackdropRightMargin) - ((int) (
            firstTickMarkRadius
                + lastTickMarkRadius))) / (tickMarkCount - 1);

    String[] tickMarkLabels = {
        getString(R.string.no_time_limit), getString(R.string.today), getString(R.string.tommorow),
    ...
    for (int i = 0; i < tickMarkCount; i++) {
      ...
      RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(tickMarkLabelWidth,
      ...
      int left = discreteSliderBackdropLeftMargin + (int) firstTickMarkRadius + (i * interval) - (
          tickMarkLabelWidth
              / 2);
        ...
        public void onImagePicked(File imageFile, EasyImage.ImageSource source, int type) {
          ...
          try {
            DataService.init().sendPhoto(reduceFile(imageFile), new DataService.onAddPhotoTask() {
              @Override public void onAddPhotoResult(UploadImageData response) {
                if (response.getResult() != null) {
                  ...
                  if (!response.getResult().imageUrl.equals("")) {
                    Picasso.with(AddTaskActivity.this)
                        .load(response.getResult().imageUrl)
                        .into(photoTaskImage);
                  }
                } else {
                  Toast.makeText(AddTaskActivity.this, R.string.error_try_again, Toast.LENGTH_SHORT)
                      .show();
                  progressImage.setVisibility(View.GONE);
                  photoTaskImage.setVisibility(View.VISIBLE);
                }
              ...
              @Override public void onErrorPhotoResult() {
                Toast.makeText(AddTaskActivity.this, R.string.error_try_again, Toast.LENGTH_SHORT)
                    .show();
                progressImage.setVisibility(View.GONE);
                photoTaskImage.setVisibility(View.VISIBLE);
