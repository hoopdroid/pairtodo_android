...
public class GiftsAdapter extends RecyclerView.Adapter<GiftsAdapter.ViewHolder> {
    ...
    public GiftsAdapter(Context context, List<Gift> giftList) {
        ...
        GiftsData data = gson.fromJson(JsonHelper.loadJSONFromAsset(context, "gifts"),
                type);
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_my_gift, viewGroup, false);
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, int i) {
        ...
        for (int j = 0; j < storageGifts.size(); j++) {
            if (giftList.get(i).template().equals(storageGifts.get(j).getTovarName())) {
                Picasso.with(context).load(storageGifts.get(j).getPreview())
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        TextView giftName;
        @Bind(R.id.giftType)
        RelativeTimeTextView giftTime;
        @Bind(R.id.previewGift)
        ImageView previewGift;
            ...
            if (v == container) {
                ...
                for (int j = 0; j < storageGifts.size(); j++) {
                    if (giftList.get(getPosition()).template().equals(storageGifts.get(j).getTovarName())) {
                        ActivityRouter.startPhotoActivity(context,
                                storageGifts.get(j).getFull(), previewGift, Constants.IMAGE_TRANSITION);
                    }
                }
