package com.pairtodopremium.data.settings;

import com.pairtodopremium.data.entities.SettingsObject;
...
public class SettingsRepository {
  ...
  public static boolean checkIfExists(Realm realm, int id) {
    ...
    RealmQuery<SettingsObject> query = realm.where(SettingsObject.class).equalTo("id", id);
  ...
  public static boolean addUserSettings(Realm realm, int userId, String token, String userPhone) {
    ...
    SettingsObject postRealmModel = new SettingsObject(userId, token, userPhone);
  ...
  public static UserInfo getUserInfo(Realm realm) {
    RealmResults<UserInfo> realmQuery = realm.where(UserInfo.class).findAll();
  ...
  public static boolean addUserSettings(Realm realm, int userId, String token) {
    ...
    realm.beginTransaction();
    SettingsObject postRealmModel = new SettingsObject(userId, token);
  ...
  public static SettingsObject getUserSettings(Realm realm) {
    RealmResults<SettingsObject> realmQuery = realm.where(SettingsObject.class).findAll();
    if (realmQuery.size() > 0) {
      return realmQuery.get(0);
    } else {
      return new SettingsObject(123, "");
    }
  }
}