...
public class SettingsApiService {
    ...
    public void changeValue(final onResult listener, String parameter, String value, String token) {
        ...
        ApiManager.getApiService().changeValues(SignConfig.APP_ID, SignConfig.generateSig(args),
                SignConfig.APP_LANG, SignConfig.OS, map, token, timeStamp).enqueue(new Callback<EmptyResultResponse>() {
            @Override
    ...
    public void logout(final onResult listener, String token) {
        ...
        ApiManager.getApiService().logout(SignConfig.APP_ID, SignConfig.generateSig(args),
                SignConfig.APP_LANG, SignConfig.OS, token, timeStamp).enqueue(new Callback<EmptyResultResponse>() {
            @Override
            public void onResponse(Call<EmptyResultResponse> call, Response<EmptyResultResponse> response) {
                listener.onSignUpResult(response.body());
            }
    ...
    public void removeCouple(final onResult listener, String token) {
        ...
        ApiManager.getApiService().removeCouple(SignConfig.APP_ID, SignConfig.generateSig(args),
                SignConfig.APP_LANG, SignConfig.OS, token, timeStamp).enqueue(new Callback<EmptyResultResponse>() {
            @Override
            public void onResponse(Call<EmptyResultResponse> call, Response<EmptyResultResponse> response) {
                listener.onSignUpResult(response.body());
